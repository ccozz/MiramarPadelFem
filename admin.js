...
function tournamentForm(t = editingTournament) {
  ...
  return `<section class="admin-card"><p class="kicker">TORNEOS</p>${isEdit ? '<h1>Modificar torneo</h1>' : open}<form id="tournament-form" class="admin-grid">
    ...
    <label>Precio<input type="number" name="price" min="0" step="1" value="${t?.registration_price ?? ''}"></label><label>WhatsApp<input name="whatsapp" type="tel" inputmode="numeric" pattern="[0-9]+" value="${esc(t?.whatsapp_number)}" required></label>
    ...
    ${isEdit ? '<p class="admin-note">Las categorías no se editan aquí para proteger las parejas ya asociadas.</p>' : '<label class="wide">Categorías <small>Separadas por coma</small><input name="categories" required></label>'}
...
async function renderTournaments() {
  const { data, error } = await sb.from('tournaments').select('id,title,start_date,end_date,location,registration_price,whatsapp_number,status,description,tournament_categories(name)').order('start_date');
...
async function renderPairs() {
  ...
  pairsCache = pairs || [];
  app.innerHTML = pairForm(categories || []) + `<section class="admin-card"><h2>Parejas cargadas</h2><div>${error ? '<p>No se pudieron cargar las parejas.</p>' : pairsCache.map(p => `<article class="admin-row"><div><b>${esc(p.player_one_alias)} / ${esc(p.player_two_alias)}</b><span>${esc(p.tournament_categories?.tournaments?.title || '')} · ${esc(p.tournament_categories?.name || '')} · ${esc(p.status)}</span></div><div><button data-edit-pair="${p.id}">Editar</button><button data-delete-pair="${p.id}">Eliminar</button></div></article>`).join('') || '<p>Sin parejas cargadas.</p>'}</div></section>`;
}
...
document.addEventListener('submit', async event => {
  ...
  if (event.target.id === 'tournament-form') { event.preventDefault(); const form = event.target; const d = Object.fromEntries(new FormData(form)); const msg = form.querySelector('.form-message'); if (d.endDate < d.startDate) { msg.textContent = 'El cierre no puede ser anterior al inicio.'; return; } if (!numeric(d.whatsapp)) { msg.textContent = 'WhatsApp solo admite números.'; return; }
    const values = { title:d.title.trim(), location:d.location.trim(), start_date:d.startDate, end_date:d.endDate, registration_price:d.price || null, whatsapp_number:d.whatsapp, status:d.status, description:d.description.trim() };
    ...
    if (!editingTournament) { const categories = String(d.categories).split(',').map(name => name.trim()).filter(Boolean).map(name => ({ tournament_id:result.data.id, name })); if (categories.length) await sb.from('tournament_categories').insert(categories); }

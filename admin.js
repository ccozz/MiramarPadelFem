...
function tournamentForm(t = editingTournament) {
  ...
  return `<section class="admin-card"><p class="kicker">TORNEOS</p>${isEdit ? '<h1>Modificar torneo</h1>' : open}<form id="tournament-form" class="admin-grid">
    ...
    <label class="wide">Categorías <small>Separadas por coma. Podés renombrar o agregar; las categorías con parejas no se eliminan automáticamente.</small><input name="categories" value="${esc((t?.tournament_categories || []).map(category => category.name).join(', '))}" required></label>
...
document.addEventListener('submit', async event => {
  ...
  if (event.target.id === 'tournament-form') { event.preventDefault(); const form = event.target; const d = Object.fromEntries(new FormData(form)); const msg = form.querySelector('.form-message'); if (d.endDate < d.startDate) { msg.textContent = 'El cierre no puede ser anterior al inicio.'; return; } if (d.registrationCloseDate && d.registrationCloseDate > d.startDate) { msg.textContent = 'El cierre de inscripciones debe ser el día de inicio o anterior.'; return; } if (!numeric(d.whatsapp)) { msg.textContent = 'WhatsApp solo admite números.'; return; }
    ...
    const categories = String(d.categories).split(',').map(name => name.trim()).filter(Boolean); if (!editingTournament) { if (categories.length) await sb.from('tournament_categories').insert(categories.map(name => ({ tournament_id:result.data.id, name }))); } else { const existing = editingTournament.tournament_categories || []; await Promise.all(existing.slice(0, categories.length).map((category, index) => sb.from('tournament_categories').update({ name: categories[index] }).eq('id', category.id))); const additions = categories.slice(existing.length).map(name => ({ tournament_id:result.data.id, name })); if (additions.length) await sb.from('tournament_categories').insert(additions); }
